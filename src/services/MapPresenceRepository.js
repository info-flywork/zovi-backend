  ...
  mapFriendRow(row) {
    ...
    if (row.check_in_id) {
      ...
      activeCheckIn = {
        ...
        stampSlug: Number(row.check_in_is_founder) ? 'founder' : null,
        titleLabel,
    ...
    return {
      ...
      // Pair lifestyle streak with the viewer (Snapchat-style).
      streakCount: Number(row.pair_streak_count || 0),
      lat: Number(row.lat),
      lng: Number(row.lng),
      distanceMeters,
      locationText: row.location_label || row.location_text || '',
